  ...
  if (behaviorList.length === 0) {
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <Link href="/abc/" className="inline-block text-orange-600 font-medium hover:underline">
          前往 ABC 分析
        </Link>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-earth-200 p-4 space-y-2">
        <div className="flex flex-wrap gap-2">
          {behaviorList.map(b => (
            <span key={b.id} className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-orange-50 text-orange-700 text-sm font-medium border border-orange-200">
              <span>{b.emoji}</span>
              {b.name}
            </span>
          ))}
      ...
      {allTechIds.length > 0 && (
        <div className="bg-orange-50 rounded-xl border border-orange-200 p-4">
          <h3 className="font-bold text-orange-700 mb-2">📋 相關訓練技術一覽</h3>
          <div className="flex flex-wrap gap-2">
            {allTechIds.map(tid => {
              ...
              return (
                <Link
                  ...
                  className="text-xs px-2.5 py-1.5 rounded-lg bg-white border border-orange-200 text-orange-600 hover:border-orange-400 transition-colors"
                >
                  {tech.name}
                </Link>
              )
